...
Ciencias::Ciencias(void)
{
	int n;
	for(n=0;n<NOTASCIENCIAS;n++) this->notas[n]=0.0f;
}

void Ciencias::setNota(int position, float nota)
{
	this->notas[position]=nota;
}

float Ciencias::getNotamedia(void)
{
	int n;
	float total;
	total = 0.0f;
	for(n=0;n<NOTASCIENCIAS;n++) total+=this->notas[n];
	return(total/(float)NOTASCIENCIAS);
}
