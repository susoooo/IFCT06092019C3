...
// M=N=1000 :: Error creating thread number 24000. Quit!!./threads10-4  0,45s user 1,02s system 157% cpu 0,935 total
...
int main(int argc, char *argv[])
{
  ...
  for (row=0; row<M; row++) {
    for (col=0; col<N; col++) {
      pos.row = row;
      pos.col = col;
      if (pthread_create(&threads[row], NULL, threadCol, (void*) &pos)){
        printf("Error creating thread number %ld. Quit!!", row*col);
        exit(1);
      }
    }
  }
...
void * threadCol(void *pos){
  struct position *p = (struct position *) pos;
  int iter;
  long total;
  int size;
  ...
  for(iter=0; iter<size; iter++){
    total += matrixA[p->row][iter] * matrixB[iter][p->col];
  }
  matrixP[p->row][p->col] = total;
}
